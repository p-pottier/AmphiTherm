...
ui <- navbarPage(
  ...
  header = tags$head(
    tags$style(HTML("
      ...
      /* Compact row padding + no wrapping + ellipsis */
      .dataTable td {
  ...
  tabPanel("Data summary",
           sidebarLayout(
             ...
             mainPanel(
               plotOutput("mapIucn", height = "650px"),
...
server <- function(input, output, session) {
  ...
  output$mapIucn <- renderPlot({
    ...
    map <- ggplot() +
      ...
      geom_hline(yintercept=c(-23.43663,0,23.43663),
                 colour="gray", linetype="dashed", size=0.5) +
      ...
      geom_point(data=dfm,
                 ...
                 shape=21, color="black", alpha=0.7, size=4) +
      scale_fill_manual(
        name="Metric",
        values=c("LTL"="#2a9d8f","PBT"="#ffb703","UTL"="#e63946"),
        guide=guide_legend(title.position="top", title.hjust=0, override.aes=list(size=5))
      ) +
      ...
      annotation_north_arrow(
        style = north_arrow_nautical(text_size=12, fill=c("grey20","white")),
    ...
    iucn_plot <- ggplot(iucn_df, aes(x=1, fill=IUCN_status)) +
      geom_bar(width=0.15) +
    ...
    (map)/(iucn_plot) + plot_layout(heights=c(5,0.7))
  })
  ...
  output$jitterMetric <- renderPlot({
    ...
    ggplot(dfm, aes(x=metric_group, y=mean_trait, fill=metric_group)) +
      ...
      stat_summary(aes(color=metric_group), fun.data=mean_sd_df, geom="pointrange",
                   size=1.2, position=position_nudge(x=0.3), show.legend=FALSE) +
      scale_fill_manual(values=c("LTL"="#2a9d8f","PBT"="#ffb703","UTL"="#e63946")) +
